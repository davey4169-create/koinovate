...
export async function POST(request) {
  try {
    ...
    if (gateway === 'paystack') {
      if (!reference) {
        return NextResponse.json(
          { error: 'Reference is required for Paystack.' },
          { status: 400 }
        )
      }
      ...
      if (result.status !== 'success') {
        return NextResponse.json(
          { error: 'Paystack payment was not successful.' },
          { status: 400 }
        )
      }
      ...
      if (!transactionId) {
        return NextResponse.json(
          { error: 'Transaction ID is required for Flutterwave.' },
          { status: 400 }
        )
      }
    ...
    if (!userId || !tier) {
      return NextResponse.json(
        { error: 'Invalid payment metadata. Missing user_id or tier.' },
        { status: 400 }
      )
    }
