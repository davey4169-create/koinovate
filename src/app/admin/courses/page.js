  ...
  return (
    <div className="admin-container">
      ...
      <div className="admin-content">
          ...
          {showForm && (
            ...
            >
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: '20px' }}>
                  <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500', color: '#333' }}>
                    Redirect URL (External course link)
                  </label>
                  <input
                    type="url"
        ...
        >
          <h2 style={{ margin: '0 0 20px 0', color: '#333' }}>All Courses</h2>

          {loading ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#999' }}>Loading...</div>
          ) : courses.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#999' }}>No courses yet</div>
          ) : (
              ...
              <tbody>
                {courses.map(course => (
                  <tr key={course.id}>
                    <td>{course.title}</td>
                    <td>
                      <span className={`badge tier-${course.min_tier}`}>{course.min_tier}</span>
                    </td>
                    <td>
                      <span className={`badge ${course.status === 'active' ? 'active' : 'inactive'}`}>
                        {course.status}
                      </span>
                    </td>
                    <td>{course.order_num}</td>
                    <td>
                      <Link href={`/admin/courses/${course.id}`} className="action-link">
                        Edit
                      </Link>
                      <button
  ...
  return (
    <div className="admin-sidebar">
      ...
      <nav className="admin-nav">
        ...
        <Link href="/admin/courses" className="nav-item active">
          📚 Courses
        </Link>
